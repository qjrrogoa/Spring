...
public class ModelAttributeController {
	...
	/*
	 방법1]
	 @RequestParam Map 사용: 이때는 커맨트 객체(DTO계열 클래스) 생성 불 필요
	 	단, 체크박스 처럼 하나의 파라미터명(키값이 됨)으로 여러값이 넘어올때는 
	 	Map으로 받기 때무에 하나만 받을 수 있다.
	 */
...
//	}
	...
	/*
	 ...
	 파라미터가 많은 경우 서블릿 API보다는
	 @ModelAtttribute어노테이션을 사용하는게 유리
	 - 단, 커맨드 객체의 속성명과 파라미터명을 일치시켜야 한다.
	 @ModelAttribute는 생략 가능
	 예]
	 @ModelAttribute 커맨드 객체 타입 매개변수명
	 */
	@RequestMapping("/Annotation/ModelAttribute.do")
